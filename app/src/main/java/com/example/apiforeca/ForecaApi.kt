...
interface ForecaApi {
    ...
    fun authenticate(@Body request: ForecaAuthRequest): Single<ForecaAuthResponse> // запрос на аутентификацию RxJava
    ...
    fun getLocation(@Header("Authorization") token: String, @Path("query") query: String): Single<LocationsResponse> // запрос на название города RxJava
    ...
    fun getForecast(@Header("Authorization") token: String, @Path("location") locationId: Int): Single<ForecastResponce> // запрос на прогноз погоды RxJava
}